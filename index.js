...
// Home - list houses
 app.get('/', async (req, res) => { const houses = await House.find(); res.render('index', { houses }); });

// Detail page
 app.get('/detail/:id', async (req, res) => { const house = await House.findById(req.params.id); res.render('detail', { house }); });

// Add house form
 app.get('/add', (req, res) => { res.render('add'); });

// Create new house
 app.post('/add', upload.single('image'), async (req, res) => { const result = await cloudinary.uploader.upload(req.file.path);
...
// Edit form 
app.get('/edit/:id', async (req, res) => { const house = await House.findById(req.params.id); res.render('edit', { house }); });

// Update house
 app.post('/edit/:id', async (req, res) => { const updatedData = { name: req.body.name, location: req.body.location, price: req.body.price, description: req.body.description, amenities: req.body.amenities.split(',').map(item => item.trim()), contact: req.body.contact, };

await House.findByIdAndUpdate(req.params.id, updatedData); res.redirect(/detail/${req.params.id}); });

// Delete house
 app.post('/delete/:id', async (req, res) => { await House.findByIdAndDelete(req.params.id); res.redirect('/'); });
